...
function babyNameMetrcis(){
  for (var i = 0; i < coreMarkets.length; i++) {
    ...
    getBabyNameGATotalViews(coreMarkets[i]);
  } 
}

function getBabyNameGATotalViews(sheetName){
  ...
  var metrics = ['ga:pageviews'];
    ...
    try{
      // Fetch the report
      var report = babyNameGAGet(tableId, startDate, endDate, metrics, options);
      ...
      if(data.length){
        ...
        Logger.log(sheetName + ' Baby Name Generator Results updated');
      }else{
        Logger.log(sheetName + ' Baby Name Generator: No data found');
      }
...
function babyNameGAGet(tableId, startDate, endDate, metrics, options) {
