function foodMain(){
  ...
  getFoodCountMetrics(); //Medicine.gs
  foodUpdateDataToSheet(); //Medicine.gs
}

function getFoodCountMetrics(){
  var request = getBigQuerySqlRequest('food_count_metrics_core_markets_master');
...
function foodUpdateDataToSheet() {
  ...
  if (rows) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      cellNo = getCellNoForFood(cols[2].v);
    ...
    Logger.log('Food Results updated');
  } else {
    Logger.log('Food No rows returned.');
  }
}

function getCellNoForFood(forDataType){
  var cellNo = '';
  if(forDataType == 'food category'){
    cellNo = '18';
  }
  return cellNo;
}